...
> **注意**：Windows 平台上依赖动态库，目前 cjc 和 cjpm 实现有差异，cjc 编译时对动态库是否有`lib`前缀不敏感，cjpm 则要求动态库必须有`lib`前缀。运行时，cjc 编出的可执行文件，依赖的动态库必须加上`lib`前缀，而 cjpm 构建出的可执行文件，依赖的动态库一定不能有`lib`前缀。请务必熟悉这个规则（实际是规格问题，未来会修复），保证能正常构建和运行。
