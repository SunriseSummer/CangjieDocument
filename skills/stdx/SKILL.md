...
- 使用 `cjc` 手工编译/链接 stdx 时，需要按依赖顺序链接各 `-lstdx.xxx` 库；依赖关系详见 `libs/standard-extension/libs_stdx/libs_overview.md` 的“包依赖”表。
- 使用 `cjpm` + `bin-dependencies.path-option` 时，通常不需要手工管理 stdx 依赖顺序。
- 使用 **静态库** 且导入 `crypto` / `net` 相关包时，需要额外系统库：
  - Linux：`-ldl`
  - Windows：`-lcrypt32`
  - 详见 `cangjie-stdx-config` Skill 的平台配置说明。
