...
### 1.3 inout 参数
- 调用 CFunc 函数时，如果参数要按指针传递，可以用 `inout` 关键字修饰实参变量，它会取变量指针/地址转为 `CPointer<T>` 类型值
- 约束：仅用于 CFunc 调用，被修饰的对象须满足 `CType` 类型约束（非 `CString`），且是可变变量，不能是 不可变变量/字面量/临时值，不能来源于 `class` 实例成员
- 指针仅在调用期间有效
...
- `--library <value>` / `-l <value>` / `-l<value>`：指定要链接的库文件
  - 库文件名格式为 `lib[arg].[extension]`（如 `-l draw` 链接 `libdraw.so` 或 `libdraw.dll` 或 `libdraw.a` 等）
...
main() {
    var pPoint = Point() // 下面要取指针传递，因此不能用 let 定义
    unsafe {
        drawPoint(inout pPoint) // inout 取指针
        println(pPoint.x)  // 1
        println(pPoint.y)  // 2
    }
}
