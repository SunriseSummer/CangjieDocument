...
- **构造函数名不能与仓颉关键字同名**（如 `Bool`、`String`、`Int64` 等都是关键字）。若需要使用关键字作为构造函数名，须用反引号转义（如 `` `Bool` ``），或改用其他名称（如 `JBool`）：
  ```cangjie
  // 错误：Bool 是关键字
  // | Bool(Bool)  // 编译错误
  // 正确：使用反引号转义或改名
...
- 若需要 `==` 比较，须使用 `@Derive[Equatable]` 宏自动派生（需导入 `import std.deriving.*`）：
  ...
  if (c == Color.Red) { ... }  // 正确
  ```
- 不使用 `@Derive` 时，应通过 `match` 模式匹配来判断枚举值
