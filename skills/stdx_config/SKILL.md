...
> **说明**：使用 `cjpm run` 运行时，工具会自动配置动态库依赖路径，无需额外设置环境变量。
...
### 6.1 Linux x86_64 动态库（完整 cjpm.toml）
...
### 6.2 Linux x86_64 静态库（完整 cjpm.toml，含 crypto/net）
...
### 6.3 macOS aarch64 动态库（完整 cjpm.toml）
...
### 6.4 Windows x86_64 静态库（完整 cjpm.toml，含 crypto/net）
