...
- **模式匹配**：`match (expr) { case pattern => exprs ... case _ => exprs }` 支持值模式、类型模式、解构模式、通配符 `_`，`if-let` 可用于 `Option` 解包
...
- **集合类型**：`Array<T>(size, init)` 定长数组；`ArrayList<T>()` 动态列表，支持 `add()`、`remove()`；`HashMap<K, V>()` 哈希表，支持 `add()`、`get()`、`contains()`；遍历用 `for (item in collection)`
