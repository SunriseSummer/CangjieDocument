...
description: "仓颉语言项目管理。当需要了解如何使用 cjpm 工具创建项目(init)、配置 cjpm.toml、管理依赖(dependencies)、构建(build)、运行(run)、测试(test)、基准测试(bench)、清理(clean)、安装(install)、工作区(workspace)、交叉编译(target)、构建脚本(build.cj)、增量编译、环境变量替换等项目管理相关知识时，应使用此 Skill。"
---
