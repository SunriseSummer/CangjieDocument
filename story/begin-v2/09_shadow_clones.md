...
> 第 90 层是无尽的回廊。为了在规定时间内通过，你学会了"影分身之术" (Spawn)。你可以同时派出多个分身去探索不同的房间，这就是并发任务的核心思维。
...
main() {
    ...
    let clones = ArrayList<Future<Int64>>()
...
*   并发任务要设置合理的超时与错误处理策略，避免"分身失联"。
*   原子操作适合简单计数，复杂状态需考虑锁或消息机制。
*   并发逻辑建议配合日志与指标，便于追踪性能。

## 小试身手

1. 为 `explore` 增加"失败返回 0 经验"的分支，并在主流程统计成功次数。
2. 改造能量池逻辑，加入"能量不足时拒绝释放"的判断。
