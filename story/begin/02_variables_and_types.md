# 02. 变量与类型：打造你的 RPG 角色

在游戏开发中，我们需要定义角色的各种属性，并区分“长期属性”（如职业、阵营）与“战斗状态”（如生命、能量）。这正是学习变量和数据类型的最佳场景。让我们创建一个能体现角色成长的属性卡。

## 本章目标

*   理解可变变量 (`var`) 与不可变变量 (`let`) 的区别。
*   掌握常见基础类型与类型推断的使用方式。
*   认识强类型系统对工程稳定性的价值。

## 1. 定义角色属性 (变量与常量)

在仓颉中，有些属性是与生俱来的（如名字），我们用 `let` 定义不可变变量；有些属性是会随战斗变化的（如生命值），我们用 `var` 定义可变变量。

```cangjie
main() {
    // 角色档案
    let heroName: String = "Cangjie Hero" // 名字一旦确定就不改了
    let heroClass = "Warrior"             // 类型推断：自动识别为 String

    // 战斗属性
    var level: Int64 = 1                  // 等级会提升
    var hp: Float64 = 100.0               // 生命值可能会有小数（如受到持续伤害）
    var isAlive: Bool = true              // 角色状态

    println("=== 角色创建成功 ===")
    println("姓名: " + heroName)
    println("职业: ${heroClass}")
    println("等级: ${level}")
    println("生命值: ${hp}")

    // 经历一场战斗...
    println("\n>>> 遭遇史莱姆！")
    hp = hp - 15.5
    level = level + 1

    println("=== 战斗结算 ===")
    println("当前生命: ${hp}")
    println("当前等级: ${level}")
}
```

## 2. 数据类型详解

通过上面的例子，我们接触了仓颉的核心基础类型，并开始形成“变量有语义、类型有边界”的思维方式：

*   **整数 (Integer)**: `Int64` 是最常用的整数类型。用于表示等级、金币数量等。
*   **浮点数 (Float)**: `Float64` 用于表示需要精度的数值，如暴击率、生命值。
*   **布尔 (Bool)**: `true` 或 `false`，用于判断状态（如 `isAlive`）。
*   **字符串 (String)**: 文本信息，如名字、对话。
*   **字符 (Rune)**: 单个字符，如 `'A'`, `'中'`。

## 3. 类型安全

仓颉是强类型语言，这意味着你不能把“生命值”（数字）赋值给“名字”（字符串），这保护了你的程序不会出现低级错误。

```cangjie
// heroName = 100 // 编译错误！不能将整数赋值给字符串变量
```

这种严谨性确保了大型项目的稳定性。

## 工程化提示

*   默认使用 `let`，仅在确实需要修改时再引入 `var`，避免状态蔓延。
*   选择数值类型时要明确单位与精度（等级是整数，概率/生命值可能需要小数）。
*   为业务字段建立统一命名规则，如 `currentHp`、`maxHp`，减少误解。

## 小试身手

1. 增加一个 `exp` 或 `energy` 字段，并在战斗后同步更新。
2. 把角色信息打印封装为一个函数，避免重复输出逻辑。
