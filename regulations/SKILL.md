...
description: "仓颉项目规范准则。当需要了解仓颉项目的推荐编码规范、项目结构规范、命名规范、格式化规范、错误处理规范、测试规范、并发规范、安全规范、文档规范、依赖管理规范、版本控制规范等最佳实践时，应使用此 Skill。"
---

# 仓颉项目规范准则 Skill

本规范基于仓颉语言特性、工具链能力及业界软件工程通用实践，为仓颉项目提供编码层面和项目层面的推荐准则。
...
├── cjpm.toml              # 项目配置（必须）
├── src/                   # 源代码目录
│   ├── main.cj            # 程序入口（executable 项目）
│   ├── module_a/          # 子包目录，与 package 声明一致
│   │   ├── foo.cj
│   │   └── bar.cj
│   └── module_b/
│       └── baz.cj
...
- 提交信息使用统一格式，推荐 Conventional Commits 规范：
  - `feat: 添加用户登录功能`
  - `fix: 修复空指针异常`
  - `refactor: 重构数据库连接池`
  - `test: 补充用户服务单元测试`
  - `docs: 更新 API 文档`
  - `chore: 升级依赖版本`
- 每次提交聚焦一个逻辑变更，避免混合多个不相关改动。
- 提交前运行 `cjfmt` 和 `cjlint`，确保代码符合规范。

### 12.3 分支策略

- `main`/`master`：稳定的发布分支，始终保持可构建和可部署。
- `develop`：开发集成分支，合并功能分支后在此验证。
- `feature/<name>`：功能开发分支，从 `develop` 创建，完成后合并回 `develop`。
- `fix/<name>`：缺陷修复分支。
- `release/<version>`：发布准备分支，用于最终测试和版本号更新。
