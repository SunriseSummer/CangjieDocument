...
| `type=macro_def` | 宏定义标记 | 标记该代码块为宏包定义，框架自动创建多模块结构 |
...
### 2.6 宏包项目 (`type=macro_def` 参数)

当文档中的示例涉及宏定义和宏调用时，使用 `type=macro_def` 标记宏定义代码块。框架会自动创建多模块 `cjpm` 项目结构：
...
<!-- check:run project=macro_demo type=macro_def -->
` ``cangjie
...
```markdown
<!-- check:compile_error project=bad_macro type=macro_def -->
` ``cangjie
macro package my_macros
// 宏定义...
` ``
...
6. 宏定义代码块使用 `type=macro_def` 参数，框架自动创建多模块项目。
...
5. **项目生成**：为每个测试用例创建独立的 `cjpm` 项目目录，包括：
   ...
   - 宏项目（含 `type=macro_def`）自动创建多模块结构
...
- **宏项目结构**：含 `type=macro_def` 的项目自动创建多模块结构，宏子模块的 `cjpm.toml` 包含 `compile-option = "--compile-macro"`，主项目通过 `path` 依赖引用宏模块。
